...
A client I worked on recently has an interesting mega menu, both visually and technically (worthy of another post). It is probably one of the most intense menus I've worked on.
...
<Figure caption="An exaggerated example of the frustration a user may feel when a user interface does not work the way it is intended." src="/src/assets/images/posts/it-crowd.gif" alt="Character Moss from the show The IT Crowd, throws a monitor in frustration." width="498" height="286" />
...
Enter my secret weapon (you probably saw it coming): SVG! Since we know the coordinates of the parent element and the mega menu, we can translate those coordinates into a vector shape and put it on the page. Voilà! Every time someone hovers over a parent item, we can use JavaScript to create an SVG on the fly with the coordinates we need. Below is a snippet from the site that actually creates the SVG.
...
<YouTube src="https://www.youtube.com/embed/o0NtjY17v5w" title="Building Better Interfaces talk by Hakim El Hattab at CSS Day 2019" />
